...
- **テキスト対応**: システムフォント自動検出、`text-anchor`、`tspan` サポート
...
| テキスト | `<text>`, `<tspan>` |
| グループ | `<g>`（子要素を再帰描画） |
| スタイル | `fill`, `stroke`, `stroke-width`, `opacity`, `fill-opacity`, `stroke-opacity`, `font-family`, `font-size`（単位付き対応）, `font-style`, `font-weight`, `text-anchor` |
...
- グラデーション（`<linearGradient>`, `<radialGradient>`）は未対応
- パターン・フィルタ・クリッピングパスは未対応
- `<use>` 要素による参照は未対応
